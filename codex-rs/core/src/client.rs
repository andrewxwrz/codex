...
impl ModelClientSession {
    ...
    ///
    /// This path is used when the provider is configured with
    /// `WireApi::Chat`. It reuses the same `ResponseStream`/`ResponseEvent`
    /// contract as the Responses path, so the downstream turn/tool/reasoning
    /// machinery is protocol-agnostic. Chat-specific constraints are enforced
    /// here rather than silently translated: `output_schema` is unsupported,
    /// and only plain `function` tools are sent (hosted/namespaced tools are
    /// excluded by the chat tool serializer).
    async fn stream_chat_completions(
        ...
        loop {
            ...
            .conversation_id(Some(responses_metadata.session_id.clone()))
            .session_source(Some(self.client.state.session_source.clone()))
            .build()
