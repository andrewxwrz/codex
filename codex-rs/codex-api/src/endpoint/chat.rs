...
//!
//! Recovered from the removed `wire_api = "chat"` path (commit d2394a2494,
//! PR #10157) and rebuilt on today's `EndpointSession` transport so it shares
//! retry, telemetry, and streaming plumbing with the Responses client. Both
//! clients emit the same `ResponseStream`/`ResponseEvent` types, which keeps
//! the downstream Codex machinery protocol-agnostic.
